...
class AbrigoAnimais {
  ...
  pessoaPodeAdotarLoco(brinquedosPessoa, brinquedosLoco) {
    ...
    return brinquedosLoco.every((brinquedo) =>
      brinquedosPessoaSet.has(brinquedo)
    );
  }

  encontraPessoas(brinquedosPessoa1, brinquedosPessoa2, ordemAnimais) {
    const listaBrinquedosP1 =
      brinquedosPessoa1 === ""
        ? []
        : brinquedosPessoa1.split(",").map((b) => b.trim());
    const listaBrinquedosP2 =
      brinquedosPessoa2 === ""
        ? []
        : brinquedosPessoa2.split(",").map((b) => b.trim());
    const animaisParaConsiderar =
      ordemAnimais === "" ? [] : ordemAnimais.split(",").map((a) => a.trim());

    if (
      new Set(listaBrinquedosP1).size !== listaBrinquedosP1.length ||
      new Set(listaBrinquedosP2).size !== listaBrinquedosP2.length
    ) {
    ...
    for (const nomeAnimal of animaisParaConsiderar) {
      ...
      if (nomeAnimal === "Loco" && animaisParaConsiderar.length > 1) {
        podeAdotarP1 = this.pessoaPodeAdotarLoco(
          listaBrinquedosP1,
          animal.brinquedos
        );
        podeAdotarP2 = this.pessoaPodeAdotarLoco(
          listaBrinquedosP2,
          animal.brinquedos
        );
      } else {
        podeAdotarP1 = this.pessoaPodeAdotar(
          listaBrinquedosP1,
          animal.brinquedos
        );
        podeAdotarP2 = this.pessoaPodeAdotar(
          listaBrinquedosP2,
          animal.brinquedos
        );
      }

      const pessoa1AtingiuLimite =
        adocoesPessoa1.length >= MAX_ANIMAIS_POR_PESSOA;
      const pessoa2AtingiuLimite =
        adocoesPessoa2.length >= MAX_ANIMAIS_POR_PESSOA;
